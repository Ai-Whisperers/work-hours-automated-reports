...
def run(
    ...
    output: Path = typer.Option(
        Path("report.xlsx"), "--output", "-o", help="Output file path"
    ),
    format: OutputFormat = typer.Option(
        OutputFormat.excel, "--format", "-f", help="Output format"
    ),
    users: Optional[List[str]] = typer.Option(
        None, "--user", "-u", help="Filter by user ID (can be specified multiple times)"
    ),
    ...
    with Progress(
        ...
        try:
            ...
            if response.success:
                ...
                table.add_row(
                    "Matched Entries",
                    f"{response.matched_entries} ({response.matched_entries/response.total_entries*100:.1f}%)",
                )
...
@app.command()
def cache(action: str = typer.Argument(..., help="Action to perform: clear, stats")):
    """Manage cache."""
    settings = get_settings()
